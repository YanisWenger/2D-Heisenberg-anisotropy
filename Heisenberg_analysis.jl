...
for l in eachindex(L)
    ...
    for d in eachindex(D)
        dd = D[d]
        for t in eachindex(T_for_DL[(ll, dd)])
            tt = T_for_DL[(ll, dd)][t]
        ...
        SwapAccept[l,d] = load("Data/$(L)_$N$(folder_name_end)/swap_$(ll)_$(dd)_$(T_for_DL[(ll, dd)][1]).jld2")[:"SwapAccept"]
    end
    print(ll, "     ")
end
...
plotL(L, T_for_DL, D, E, 23, "E", EΔ)
plotL(L, T_for_DL, D, C, 10, "C", CΔ)
...
# plotD(L, T_for_DL, D, C, "C", CΔ2, 3)   # with error propag from ΔE, smaller error bars
plotD(PlotColors, L, T_for_DL, D, C, "C", CΔ)
...
Tmax_χ, χmax, χfitln, χfitln_err, χfitpower, χfit_Tc = Plot_Max_C_Χ(PlotColors, D, T_for_DL, χ, χΔ, L, "Susc") # Plot and fit the max of C or χ
Tmax_C, Cmax, Cfitln, Cfitln_err, Cfitpower, Cfit_Tc = Plot_Max_C_Χ(PlotColors, D, T_for_DL, C, CΔ, L, "C")
...
scaling_plot_C_Ising(L, T_for_DL, D, C, 10, Tmax_C, CΔ)
...
for d=1:23
    p=Plots.plot(T_for_DL[L[6],D[d]], C[6,1:length(T_for_DL[L[6],D[d]]),d]/maximum(C[6,:,d]), label=D[d])
    Plots.plot!(T_for_DL[L[6],D[d]], χ[6,1:length(T_for_DL[L[6],D[d]]),d]/maximum(χ[6,:,d]))
    display(p)
end

Tpic=zeros(Float32, nD)
for d in eachindex(D)
    Tpic[d]=T_for_DL[(L[l], D[d])][argmax(C[l,:,d])]
end
@save "Tpic_$(L[l]).jld2" Tpic D
...
Gaussmax(T_for_DL[100, 0.01f0],C[7,:,13])
T_for_DL[100,.01f0][argmax(C[7,:,13])]
...
Plots.plot(T_for_DL[100, 0.01f0][36:43], filter(!iszero, C[7,:,13][36:43]), xaxis="Temperature", yaxis="Heat capacity", label="L=100, d=0.01")