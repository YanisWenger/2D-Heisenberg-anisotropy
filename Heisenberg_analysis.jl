...
PlotColors = cgrad([RGB(.5,.9,1), RGB(0,0,.5), RGB(0,0,0), RGB(.6,0,0), RGB(1,.75,.75)], [0., .4999, .5, .5001, 1.], categorical = false) # Colors for plots
...
ξ, σξ = corrlength(ConnCorr, T_for_LD, L, D_for_L, 100, -1000f0, 0, false, true, "Connected Correlation function")           # sixth entry for the wanted temperature, seventh to plot or not, eighth component to "true" to get exponent of the algebraic decay instead of correlation length
...
Fit_ξ(L, D_for_L, T_for_LD, ConnCorr, 200, .05f0)
ξ_vs_T(PlotColors, L, D_for_L, T_for_LD, ConnCorr, 100, "Ising")
...
Plots.plot(χ_Tc_keys, [x[1][1] for x in χ_Tc_vals], seriestype=:scatter, yerr=[x[2][1] for x in χ_Tc_vals], label="\$\\chi_{max}\$", title = "Critical exponent", framestyle = :box, xlabel="⬅ Ising                Value of D                XY ➡", ylabel="Critical exponent", ylim = (-2.1,-.1))
Plots.plot!(C_Tc_keys, [x[1][1] for x in C_Tc_vals], seriestype=:scatter, yerr=[x[2][1] for x in C_Tc_vals], label="\$C_{max}\$")
...
scaling_plot_χ_Heisenberg(L, T_for_LD, D_for_L, χ, 0, Tmax_χ, χΔ, χmax)
